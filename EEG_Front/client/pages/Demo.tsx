...
export default function Demo() {
  ...
  const [audioFiles] = useState({
    demoIntroduction: new Audio('/demo_introduction.mp3'),
    comprehensiveTestGuide: new Audio('/comprehensive_test_guide.mp3'),
    simpleTestGuide: new Audio('/simple_test_guide.mp3')
  });
  ...
  // 각 상황별 재생 함수들
  const playDemoIntroduction = () => playAudio(audioFiles.demoIntroduction);
  const playComprehensiveTestGuide = () => playAudio(audioFiles.comprehensiveTestGuide);
  const playSimpleTestGuide = () => playAudio(audioFiles.simpleTestGuide);
  ...
  const speakPageIntroduction = () => {
    playDemoIntroduction();
  };

  // 검사 안내 음성 안내
  const speakTestGuide = () => {
    playComprehensiveTestGuide();
  };
