...
export default function Assessment() {
  ...
  const [audioFiles] = useState({
    // Setup 단계
    welcomeMessage: new Audio('/welcome_message.mp3'),
    beforeStartChecklist: new Audio('/before_start_checklist.mp3'),
    serialNumberGuide: new Audio('/serial_number_guide.mp3'),
    serialInputComplete: new Audio('/serial_input_complete.mp3'),
    guideViewGuide: new Audio('/guide_view_guide.mp3'),
    guideButtonHint: new Audio('/guide_button_hint.mp3'),
    
    // Preparation 단계
    deviceWearGuide: new Audio('/device_wear_guide.mp3'),
    deviceChecklist: new Audio('/device_checklist.mp3'),
    electrodePlacement: new Audio('/electrode_placement.mp3'),
    connectionSuccess: new Audio('/connection_success.mp3'),
    connectionFailed: new Audio('/connection_failed.mp3'),
    
    // Recording 단계
    measurementStart: new Audio('/measurement_start.mp3'),
    measurement1min: new Audio('/measurement_1min.mp3'),
    measurement2min: new Audio('/measurement_2min.mp3'),
    measurementAlmostDone: new Audio('/measurement_almost_done.mp3'),
    measurementComplete: new Audio('/measurement_complete.mp3'),
    
    // Processing 단계
    analysisStart: new Audio('/analysis_start.mp3'),
    analysisProgress: new Audio('/analysis_progress.mp3'),
    
    // Complete 단계
    testComplete: new Audio('/test_complete.mp3'),
    nextStepGuide: new Audio('/next_step_guide.mp3'),
    
    // 에러 상황
    errorRetry: new Audio('/error_retry.mp3'),
    audioPlayFailed: new Audio('/audio_play_failed.mp3')
  });
  ...
  useEffect(() => {
    ...
    const handleAudioError = () => setIsSpeaking(false);
  ...
  // 각 상황별 재생 함수들
  const playWelcomeMessage = () => playAudio(audioFiles.welcomeMessage);
  const playBeforeStartChecklist = () => playAudio(audioFiles.beforeStartChecklist);
  const playSerialNumberGuide = () => playAudio(audioFiles.serialNumberGuide);
  const playSerialInputComplete = () => playAudio(audioFiles.serialInputComplete);
  const playGuideViewGuide = () => playAudio(audioFiles.guideViewGuide);
  const playGuideButtonHint = () => playAudio(audioFiles.guideButtonHint);

  const playDeviceWearGuide = () => playAudio(audioFiles.deviceWearGuide);
  const playDeviceChecklist = () => playAudio(audioFiles.deviceChecklist);
  const playElectrodePlacement = () => playAudio(audioFiles.electrodePlacement);
  const playConnectionSuccess = () => playAudio(audioFiles.connectionSuccess);
  const playConnectionFailed = () => playAudio(audioFiles.connectionFailed);

  const playMeasurementStart = () => playAudio(audioFiles.measurementStart);
  const playMeasurement1min = () => playAudio(audioFiles.measurement1min);
  const playMeasurement2min = () => playAudio(audioFiles.measurement2min);
  const playMeasurementAlmostDone = () => playAudio(audioFiles.measurementAlmostDone);
  const playMeasurementComplete = () => playAudio(audioFiles.measurementComplete);

  const playAnalysisStart = () => playAudio(audioFiles.analysisStart);
  const playAnalysisProgress = () => playAudio(audioFiles.analysisProgress);

  const playTestComplete = () => playAudio(audioFiles.testComplete);
  const playNextStepGuide = () => playAudio(audioFiles.nextStepGuide);

  const playErrorRetry = () => playAudio(audioFiles.errorRetry);
  const playAudioPlayFailed = () => playAudio(audioFiles.audioPlayFailed);
  ...
  const speakChecklist = () => {
    playBeforeStartChecklist();
  };

  // 시리얼 넘버 안내 음성 함수 추가
  const speakSerialGuide = () => {
    playSerialNumberGuide();
  };

  // 가이드 보기 버튼 안내 음성 함수 추가
  const speakGuideButton = () => {
    playGuideViewGuide();
  };
  ...
  useEffect(() => {
    if (currentStep === "recording" && isRecording) {
      if (recordingTime === 60) {
        ...
        playMeasurementAlmostDone();
      } else if (recordingTime === 180) {
        playMeasurementComplete();
      }
    }
  }, [recordingTime, currentStep, isRecording]);
  ...
  }, [isRecording, recordingTime, currentStep]);
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() => {
                        setShowSerialGuide(!showSerialGuide);
                        // 가이드를 보여줄 때 음성 안내도 함께 재생
                        if (!showSerialGuide && isVoiceMode && isTTSEnabled) {
                          speakGuideButton(); // 가이드 보기 버튼 안내 음성 재생
                        }
                      }}
                  ...
                  <div className="space-y-3">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        // 시리얼 넘버 입력 시 가이드 버튼 힌트 음성 안내
                        if (value.trim() && !showSerialGuide) {
                          // 음성 모드이고 TTS가 활성화된 경우 가이드 버튼 힌트 재생
                          if (isVoiceMode && isTTSEnabled) {
                            setTimeout(() => {
                              playGuideButtonHint();
                            }, 500); // 입력 완료 후 잠시 대기 후 힌트 재생
                          }
                        }
                      }}
