...
export default function CognitiveTest() {
  ...
  const [audioFiles] = useState({
    trailMakingGuide: new Audio('/trail_making_guide.mp3'),
    memoryGuide: new Audio('/memory_guide.mp3'),
    attentionGuide: new Audio('/attention_guide.mp3'),
    fluencyGuide: new Audio('/fluency_guide.mp3'),
    abstractionGuide: new Audio('/abstraction_guide.mp3'),
    orientationGuide: new Audio('/orientation_guide.mp3'),
    namingGuide: new Audio('/naming_guide.mp3')
  });
  ...
  // 각 상황별 재생 함수들
  const playTrailMakingGuide = () => playAudio(audioFiles.trailMakingGuide);
  const playMemoryGuide = () => playAudio(audioFiles.memoryGuide);
  const playAttentionGuide = () => playAudio(audioFiles.attentionGuide);
  const playFluencyGuide = () => playAudio(audioFiles.fluencyGuide);
  const playAbstractionGuide = () => playAudio(audioFiles.abstractionGuide);
  const playOrientationGuide = () => playAudio(audioFiles.orientationGuide);
  const playNamingGuide = () => playAudio(audioFiles.namingGuide);
  ...
  const speakQuestion = () => {
    ...
    if (currentQ.type === 'trail-making') {
      ...
      playAbstractionGuide();
    } else if (currentQ.type === 'orientation') {
      playOrientationGuide();
