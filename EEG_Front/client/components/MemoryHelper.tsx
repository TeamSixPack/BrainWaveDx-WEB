...
export default function MemoryHelper() {
  ...
  const [audioFiles] = useState({
    ...
    questionSelectionGuide: new Audio('/question_selection_guide.mp3'),
    question1: new Audio('/question_1.mp3'),
    question2: new Audio('/question_2.mp3'),
    question3: new Audio('/question_3.mp3'),
    question4: new Audio('/question_4.mp3'),
    comfortGuide: new Audio('/comfort_guide.mp3')
  });
  ...
  const playConsultationEnd = (onComplete?: () => void) => playAudio(audioFiles.consultationEnd, onComplete);
  const playConsultationComplete = (onComplete?: () => void) => playAudio(audioFiles.consultationComplete, onComplete);
  const playQuestionSelectionGuide = (onComplete?: () => void) => playAudio(audioFiles.questionSelectionGuide, onComplete);
  const playQuestion1 = (onComplete?: () => void) => playAudio(audioFiles.question1, onComplete);
  const playQuestion2 = (onComplete?: () => void) => playAudio(audioFiles.question2, onComplete);
  const playQuestion3 = (onComplete?: () => void) => playAudio(audioFiles.question3, onComplete);
  const playQuestion4 = (onComplete?: () => void) => playAudio(audioFiles.question4, onComplete);
  const playComfortGuide = (onComplete?: () => void) => playAudio(audioFiles.comfortGuide, onComplete);

  // 기존 speakMessage 함수를 새로운 함수로 대체
  const speakMessage = (message: string, onComplete?: () => void) => {
    // 메시지에 따라 적절한 음성 재생
    if (message.includes('안녕하세요') || message.includes('기억 도우미')) {
      playMemoryHelperWelcome(onComplete);
    } else if (message.includes('상담을 시작') || message.includes('대화를 나누어보세요')) {
      playConsultationStart(onComplete);
    } else if (message.includes('상담을 종료') || message.includes('종료하시겠습니까')) {
      playConsultationEnd(onComplete);
    } else if (message.includes('상담이 완료') || message.includes('수고하셨습니다')) {
      playConsultationComplete(onComplete);
    } else if (message.includes('어떤 질문에 대해') || message.includes('편하게 선택해주세요')) {
      playQuestionSelectionGuide(onComplete);
    } else {
      // 기본적으로 환영 메시지 재생
      playMemoryHelperWelcome(onComplete);
    }
  ...
  const handleQuestionSelect = (questionId: string) => {
    setSelectedQuestions(prev => {
      if (prev.includes(questionId)) {
        ...
        // 질문을 선택할 때마다 해당 질문을 읽어주기
        const question = experienceQuestions.find(q => q.id === questionId);
        if (question) {
          // 질문 ID에 따라 해당 음성 재생
          switch (questionId) {
            case 'q1':
            ...
            case 'q4':
              ...
              break;
            default:
              speakMessage(question.question);
          }
        }
